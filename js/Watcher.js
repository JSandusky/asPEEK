...
function BuildVariableAssignment(node)
{
    var title = node.data.title;
    var sub = title.indexOf(":");
    var name = title.substr(0,sub-1);
    var value = title.substr(sub+2,title.length - sub);
    
    var request = GetUpstreamValues(node.parent);
    request += "~";
    request += name;
    request += "~";
    request += value;
...
function SendLocalVariableValue(node)
{
    AssignVariable(true,BuildVariableAssignment(node));
}

function SendThisVariableValue(node)
{
    AssignVariable(false,BuildVariableAssignment(node));
}

function editNode(node, isLocal){
        var prevTitle = node.data.title,
                tree = node.tree;
                
        var localVal = isLocal;
        var sub = prevTitle.indexOf(":")+2;
        var shortName = prevTitle.substr(0,sub);
        var newText = prevTitle.substr(sub, prevTitle.length-sub);
        
        try {
            if (node.childList != null && node.childList.length > 0 == true)
                return;
        } catch (e)
        {
        }
        ...
        $(".dynatree-title", node.span).html("<input id='editNode' value='" + newText +"'>");
        ...
        $("input#editNode")
                ...
                .keydown(function(event){
                        ...
                        node.setTitle(shortName + title);
                        tree.$widget.bind();
                        node.focus();
                        // Re-enable mouse and keyboard handlling
                        if (localVal == true) {
                            SendLocalVariableValue(node);
                        } else {
                            SendThisVariableValue(node);
                        }
                });
}
...
function GetUpstreamValues(node)
{
    if (node.parent != null) {
        return GetUpstreamValues(node.parent) + "~" + node.data.title;
    }
...
function FillThis(stack,obj)
{
  var recievedObj;
  try
  {
    recievedObj = $.parseJSON(obj);
  }
  catch(e)
  {
    return;
